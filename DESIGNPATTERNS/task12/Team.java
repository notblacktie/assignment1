...
public class Team implements Callable<Integer> {
    ...
    private boolean stakeOutTrack() {
        ...
        if (nonNull(track)) {
            System.out.printf("Команда '%s' claimed track #%d for $%d\n",
    ...
    private boolean getShoes() {
        ...
        if (nonNull(shoes)) {
            ...
            System.out.printf("Command '%s' changed\n", title);
            return true;
        }
        System.out.printf("Command '%s' didn't have enough shoes\n", title);
    ...
    private int play() {
        ...
        System.out.printf("Command '%s' races bounces balls, orders food and drinks for $%d\n",
                title, spentMoney);
        try {
